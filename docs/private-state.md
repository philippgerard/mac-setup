...
The bundled backend's Filen login session is a credential and is intentionally
not copied. It stays in a pre-existing `~/.filen-cli` directory when one is
present and otherwise lives below
`~/Library/Application Support/filen-cli`. The guided provisioner restricts the
selected directory and saved-login file to user-only access, launches Filen
Menubar, and directs a clean install to the app's Login flow.
